...
data_path = "/<Insert_link_do_dataset>/"
...
kspace, sens_maps, y_shift = load_raw_data(
    f"{data_path}/mr_data/{subset}/{subject}/t2s_gre_fr.hf"
)
...
img_coil_combined, nifti_header = load_coil_combined_reconstruction(
    f"{data_path}/mr_data/{subset}/{subject}/t2s_gre_fr_recon.hf"
)
...
ref_mask = load_reference_mask(
    f"{data_path}/mr_data/{subset}/{subject}/motion_mask_t2s_gre_fr_move.txt"
)
...
brain_mask = load_segmentation(
    f"{data_path}/mr_data/{subset}/{subject}/seg_brain_reg-to-t2s.nii"
)
white_matter = load_segmentation(
    f"{data_path}/mr_data/{subset}/{subject}/seg_wm_reg-to-t2s.nii"
)
gray_matter = load_segmentation(
    f"{data_path}/mr_data/{subset}/{subject}/seg_gm_reg-to-t2s.nii"
)


"""2) Loading the motion data:"""
motion_data = load_motion_data(
    f"{data_path}/motion_data/medium_motion_cp_003.json"
)
